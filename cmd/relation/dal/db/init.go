...
func Init() {
	...
	if DB.Migrator().HasTable(Follower{}) {
		return
	}
	err = DB.Migrator().CreateTable(&Follower{})
	if err != nil {
		panic("failed to create the follower table")
	}
	if DB.Migrator().HasTable(Following{}) {
		return
	}
	err = DB.Migrator().CreateTable(&Following{})
	if err != nil {
		panic("failed to create the following table")
	}
}
